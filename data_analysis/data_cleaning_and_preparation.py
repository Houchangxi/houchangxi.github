...
# we can use join method to pass a list or tuple quickly
print('::'.join(pieces))

print('guido' in val)
print(val.index(','))
print(val.find(':'))
# index method and find method are a little different. If using index to check string, it will report a error when not finding.
# print(val.index(':'))
print(val.count(',')) #return the number of a str
print(val.replace(',','::'))
print(val.replace(',',''))
# re module function fall into 3 categories: pattern matching, substitution, splitting.
import re
#
test = 'foo    bar\t baz  \tqux'
# \s+ means one or more whitespace characters
print(re.split('\s+',test))
# re.split() is regular expression. we can use re.compile to form a reusable regex object
regex = re.compile('\s+')
print(regex.split(test))

print(regex.findall(test))

# re.compile is highly recommended. Saving CPU cycles
...
# regex.match return None, as it only will match if the pattern occurs at the start of string
...
print(data)
print(data.isnull())
# 我们可以用pandas的 str attribute去转换成string形式，然后用str的一些方法对pandas的数据进行操作
print(data.str.contains('gmail'))
# str 就可以直接用findall march search
